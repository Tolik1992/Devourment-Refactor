...
int property DIALOG_KEY = 34 auto ; G by default
...
Event OnPlayerLoadGame()
	...
	if MCM.VERSION < MCM.GetVersion()
		Log3(PREFIX, "OnPlayerLoadGame", "UPGRADE REQUIRED", MCM.VERSION, MCM.GetVersion())
		MCM.Upgrade(MCM.VERSION, MCM.GetVersion())
	endIf
...
* It listens for DIALOG_KEY and initiates dialog with prey if it's pressed.
...
* It listens for DIALOG_KEY and initiates dialog if it's pressed.
...
/;
	...
	Function onUpdate()
		if !Manager.isNoEscape(preyData)
	...
	bool Function VoreSleep()
		...
		elseif !SafeProcess()
			...
			return false
		elseif !playerRef.HasPerk(MCM.Comfy)
...
bool Function IsControllable(Actor target, bool endo)
	...
	elseif endo && (target.isInFaction(Follower) || target.IsPlayerTeammate() || target.IsPlayersLastRiddenHorse())
		return true
	elseif !endo
		return MCM.EnableCordyceps && playerRef.HasPerk(MCM.Cordyceps) && target.HasKeyword(Manager.ActorTypeNPC)
	else
		return false
	endIf
EndFunction
...
Function CheckDependencies()
	if MCM.DisableDependencyChecks
		return
	endIf
