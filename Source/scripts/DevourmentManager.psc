...
bool property UseHelpMessages = true auto
...
float property cameraShake = 0.2 auto
...
bool property noEscape = true auto
{Controls whether prey can escape automatically, or must struggle free.}
...
{ Processes live vore digestion for a pred/prey pair. }
	...
	elseif CanEscape(preyData) && timer <= 0.0
		if DEBUGGING
...
Success is automatic for prey that is dead, bleeding out, surrendered, or asleep (if the pred has silent swallow).
	...
	float healthFactor = -(preyHealth + preyStamina/2.0)
...
Event ResetActorWeight(ObjectReference f)
	DevourmentWeightManager.instance().ResetActorWeight(f as Actor)
EndEvent
...
bool Function canEscape(int preyData)
	return !noEscape && JLua.evalLuaInt("return dvt.CanEscape(args)", preyData, 0, false) as bool
EndFunction
...
float Function GetDefaultPreySkill(Actor target)
	if target == playerRef
		...
		Devourment_PreySkill.setValue(skill)
		return skill
	elseif target.HasKeyword(DevourmentSuperPrey)
		return 25.0 + 2.0 * target.getLevel()
	else
		return 6.0 + 1.7 * target.getLevel()
	endIf
...
bool Function saveSettings(String settingsFileName)
	...
	JMap.setInt(data, "PlayerAvoidant", 	PlayerAvoidant as int)
	JMap.setInt(data, "PlayerCentric", 		PlayerCentric as int)
	...
	JMap.setInt(data, "NoEscape", 				NoEscape as int)
	...
	JMap.setInt(data, "DIALOG_KEY",		PlayerAlias.DIALOG_KEY)
...
bool Function loadSettings(String settingsFileName)
	...
	PlayerAvoidant = 		JMap.getInt(data, "PlayerAvoidant", 		PlayerAvoidant as int) as bool
	PlayerCentric = 		JMap.getInt(data, "PlayerCentric", 			PlayerCentric as int) as bool
	...
	NoEscape = 				JMap.getInt(data, "NoEscape", 				NoEscape as int) as bool
	...
	PlayerAlias.DIALOG_KEY = 	JMap.getInt(data, "DIALOG_KEY",		PlayerAlias.DIALOG_KEY)
